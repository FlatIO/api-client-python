...
**max_points** | **float** | If set, the grading will be enabled for the assignment with this value as the maximum of points  | [optional] 
...
**due_date** | **datetime** | The due date of this assignment, late submissions will be marked as past due. If not set, the assignment won&#39;t have a due date.  | [optional] 
...
**class_group_ids** | **List[str]** | Optional list of specific class group IDs to apply to the assignment. When transitioning to active state with group submission mode: - If provided: Only these specific groups will be applied - If not provided: All class groups will be applied, or randomized groups created if none exist  | [optional] 
...
class_assignment_update_from_dict = ClassAssignmentUpdate.from_dict(class_assignment_update_dict)
```
