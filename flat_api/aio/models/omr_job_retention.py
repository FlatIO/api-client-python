...
class OmrJobRetention(BaseModel):
    """
    ...
    """ # noqa: E501
    expiry_date: datetime = Field(description="When this job's uploaded files and recognition results become eligible for erasure. Fixed when the job is created: changing the account's retention period does not move the deadline of jobs that already exist. ", alias="expiryDate")
    expired_date: Optional[datetime] = Field(default=None, description="When the job's stored files were actually erased. Present only once that happened.  An expired job keeps the `status` it finished with and stays listable, but its `result` is no longer served and downloads fail with `OMR_JOB_EXPIRED`. ", alias="expiredDate")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["expiryDate", "expiredDate"]
